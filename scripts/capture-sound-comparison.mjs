...
const directory = 'evidence/p2i', baseURL = process.env.SOUND_LAB_URL ?? 'http://127.0.0.1:4179'
...
try {
  for (const viewport of [{ width: 1440, height: 1000 }, { width: 390, height: 844 }]) {
    for (const variant of ['a', 'b']) {
      ...
      const context = await browser.newContext({ viewport, baseURL, recordVideo: { dir: '.checkpoints/p2i/videos', size: viewport }, hasTouch: viewport.width < 640, isMobile: viewport.width < 640 })
      ...
      async function measure(name, ms) {
        ...
        stages.push(await page.evaluate(({ name, initial }) => {
          ...
          return { name, rafCallbacks: c.raf, visualUpdates: Number(r.dataset.analysisFrames) - initial, frameIntervalP95: p95(c.intervals), callbackP95: p95(c.durations), longTasks: c.longTasks, layoutShift: c.shifts, phase: r.dataset.audioState }
        }, { name, initial }))
      }
      ...
      if (variant === 'b') {
        const area = await page.locator('.bow-contact').evaluate(el => ({ x: Number(el.dataset.x) - 48, y: Number(el.dataset.y) - 35, width: 96, height: 70 }))
      ...
      await page.waitForTimeout(250); await measure('settled-pause', 800)
  ...
  for (const fixture of [
    ...
    { width: 1440, height: 1000, name: '1440-b-medium', path: '/?compare=b&tail=medium', video: true },
