...
test('unchanged position cannot reset the native quiet clock, while actual scroll does', () => {
  ...
  const ready = { now: 5000, enteredAt: 1000, lastScroll: activity.at, lastStain: 1000, regions: 0, visible: true, reduced: false, touching: false }
...
test('hundreds of desktop samples produce only representative clocked blooms, capped at four', () => {
  let last = -Infinity, anchor = -1000, count = 0
  for (let now = 0; now <= 1900; now += 5) {
    const x = now * .65
  ...
  assert.equal(desktopWetReady({ now: 10000, lastStain: last, travel: 500, sampledAt: 10000, regions: 4 }), false)
  ...
  assert.ok(desktopWetTiming.interval >= 450 && desktopWetTiming.interval <= 700)
