...
const candidateDraft = JSON.stringify(createDraft({ ...experienceCanonical, worksLayout: 'z-depth', portrait: 'straight', magnet: false }))
...
test('normal WORKS and dev=0 ignore candidate URLs and saved selections without changing canonical HOME', () => {
  for (const query of ['', '?worksExperience=z-depth&type=albums', '?dev=0&worksExperience=z-depth&portrait=off&magnet=off&all=a']) {
...
test('WORKS development precedence is URL, then saved draft, then current canonical', () => {
  ...
  assert.equal(saved.worksLayout, 'z-depth')
  ...
  const explicit = readComparison('?dev=1&worksExperience=z-depth', false, JSON.stringify(oldHomeDraft()))
  assert.equal(explicit.worksLayout, 'z-depth')
...
test('WORKS compare/change/reset preserve project base, locale, archive filter, unrelated query and hash', () => {
  ...
  const selected = readComparison('?dev=1&worksExperience=z-depth', false)
  ...
  assert.equal(copied.searchParams.get('worksExperience'), 'z-depth')
  assert.equal(copied.searchParams.has('worksLayout'), false)
  assert.equal(copied.searchParams.has('portrait'), false)
  assert.equal(readComparison(copied.search, false).worksLayout, 'z-depth')
  ...
  for (const filter of ['albums', 'performances']) {
    const exited = new URLSearchParams(resetComparison(`?dev=1&worksExperience=z-depth&type=${filter}&campaign=return`, true))
...
test('promotion copies the WORKS candidate explicitly but does not promote it by selection or copying', () => {
  ...
  assert.equal(payload.options.worksLayout, 'z-depth')
  ...
  assert.deepEqual(result.changes, [{ key: 'worksLayout', from: 'current', to: 'z-depth' }])
  assert.equal(result.next.options.worksLayout, 'z-depth')
...
test('only the exact complete historical six-key v1 draft migrates, preserving HOME choices and its input', () => {
  ...
  assert.throws(() => validateDraft({ ...oldHomeDraft(), options: { ...oldHomeOptions, works: 'z-depth' } }))
})
...
test('invalid layout values cannot enter stored, promoted or URL-derived runtime state', () => {
  ...
  assert.equal(state.worksLayout, 'z-depth')
...
test('superseded Helix draft preserves HOME and retires the layout without mutating the payload', () => {
  const old = { schemaVersion: 1, kind: 'experience-draft', options: { ...oldHomeOptions, worksLayout: 'spatial-helix' } }
  const before = JSON.stringify(old)
  const next = validateDraft(old)
  assert.deepEqual(next.options, { ...oldHomeOptions, worksLayout: 'current' })
  assert.equal(JSON.stringify(old), before)
  assert.equal(readComparison('?dev=1&works=spatial-helix', false, before).worksLayout, 'current')
  assert.throws(() => validatePromotion({ schemaVersion: 1, kind: 'experience-promotion', options: { portrait: 'hanji', magnet: true, worksLayout: 'spatial-helix' } }))
})

test('all three candidates round-trip through URL, Draft and promotion with identical canonical isolation', () => {
  for (const worksLayout of ['z-depth', 'wave-path', 'stack-flow'] as const) {
