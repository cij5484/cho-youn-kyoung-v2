...
for(const width of [320,390,768,1440])test(`depth queue ${width}: one complete threshold, next edge behind it, native work changes`,async({page})=>{
  await page.setViewportSize({width,height:900});await ready(page)
  const scene=page.locator('.works-scene')
  for(const i of [0,1,3,4]){
    await scene.evaluate((e,i)=>scrollTo({top:scrollY+e.getBoundingClientRect().top+((e as HTMLElement).offsetHeight-e.querySelector<HTMLElement>('.works-sticky')!.offsetHeight)*i/4,behavior:'instant'}),i)
    await expect(scene).toHaveAttribute('data-ribbon-index',String(i));await expect(scene).toHaveAttribute('data-work-threshold','true')
    await expect(scene).toHaveAttribute('data-ribbon-position',`${i}.0000`)
    const card=page.locator('.selected-work').nth(i)
    await expect(card).toHaveAttribute('data-depth','0.00')
    expect(await card.evaluate(e=>getComputedStyle(e).clipPath.match(/[\d.]+/g)?.every(n=>Number(n)===0))).toBe(true)
    expect(await card.evaluate(e=>!(e as HTMLElement).inert)).toBe(true)
    if(i<4){
      const next=page.locator('.selected-work').nth(i+1)
      expect(Number(await next.getAttribute('data-depth'))).toBeLessThan(-300)
      expect(await next.evaluate(e=>getComputedStyle(e).clipPath.match(/[\d.]+/g)?.some(n=>Number(n)>0))).toBe(true)
    }
  }
  expect(await scene.evaluate(e=>(e as HTMLElement).offsetHeight/e.querySelector<HTMLElement>('.works-sticky')!.offsetHeight)).toBeLessThanOrEqual(2.51)
...
test('afterimages carry source material over short boundaries and disappear in reduced motion',async({page})=>{
  ...
  for(const [selector,offset,state] of [['.album-object-scene',.58,'work-plane'],['.stage-artist-sequence',.5,'album-light'],['.home-outro',.66,'artist-edge']] as const){
  ...
  await expect(page.locator('.work-plane-memory')).toHaveAttribute('data-asset-id','asset:home-sanjo')
