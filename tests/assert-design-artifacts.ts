...
export async function assertDesignArtifacts(targetName: BuildTargetName) {
  ...
  async function inspect(directory: string) {
    for (const entry of await readdir(directory, { withFileTypes: true })) {
      ...
      for (const marker of [...markers, 'P2I_SOUND_COMPARISON_LAB_ONLY', 'bow-contact', 'contact-motion', 'sound-comparison', 'bow-features/1', 'HOME_SIGNATURE', '__test-audio']) assert.ok(!bytes.includes(Buffer.from(marker)), `Lab content leaked: ${path}`)
