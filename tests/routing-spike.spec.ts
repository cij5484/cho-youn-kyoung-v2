...
for (const fixture of siteRoutes) {
  test(`hydrated ${fixture.path}: direct, hard refresh, semantic counterpart and history`, async ({ page, context }, info) => {
    ...
    if (fixture.key === 'works') {
      ...
      // This is the legacy Framework fixture, not the shipped EditionApp WORKS.
      // Keep its routing/hydration/history contract. Current product filtering is
      // exercised against the built artifact in production-works.spec.ts.
      await page.locator('.editorial-navigation .nav-signature').click()
      await checkMetadata(page, siteRoutes[0], info)
      await page.goBack()
      await checkMetadata(page, fixture, info)
