...
export function GlobalAudioPlayer() {
  ...
  return <><AudioSignature/>{createPortal(<>
    {album && <aside className="album-mini-player" data-playing={playing} aria-label="앨범 플레이어">
      ...
      <div ref={volumeControl} className="album-volume" onPointerEnter={event => { if (event.pointerType === 'mouse' && !event.currentTarget.querySelector('.album-volume-open')?.getClientRects().length) setVolumeOpen(true) }}
        onPointerLeave={event => { if (event.pointerType === 'mouse' && !event.buttons && !event.currentTarget.querySelector('.album-volume-open')?.getClientRects().length && !event.currentTarget.contains(document.activeElement)) setVolumeOpen(false) }}
        onFocus={event => { if (!event.currentTarget.contains(event.relatedTarget) && event.target.matches(':focus-visible')) setVolumeOpen(true) }}
        ...
        <button aria-label={volume === 0 ? '음소거 해제' : `음소거, 현재 음량 ${Math.round(volume * 100)}%`} onClick={() => globalPlayback.toggleMute()}>
        ...
        <button className="album-volume-open" aria-label="음량 조절 열기" aria-expanded={volumeOpen} aria-controls="album-volume-panel" onClick={() => setVolumeOpen(true)}>음량</button>
