...
export function DevelopmentComparison({ settings, onChange, onJump, onReset, onLoad, onExit, onClose, saveMessage }: {
  settings: ComparisonSettings; onChange: (patch: ComparisonPatch) => void; onJump: (scene: 'sound' | 'performance' | 'artist') => void;
  ...
  async function copy(promotion = false) {
    const payload = promotion ? JSON.stringify(createPromotion(settings), null, 2) : comparisonAddress(location.href, settings)
  ...
  return <aside className="development-comparison" id="development-comparison" aria-label="개발 비교" lang="ko">
    ...
    <table className="development-values"><caption>현재 배포값과 개발 선택</caption><thead><tr><th scope="col">항목</th><th scope="col">현재 배포</th><th scope="col">개발 선택</th></tr></thead><tbody>{experienceKeys.map(key => <tr key={key} data-changed={settings[key] !== experienceCanonical[key]}>
