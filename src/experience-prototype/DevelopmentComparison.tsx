...
export function DevelopmentComparison({ settings, onChange, onJump, onReset, onLoad, onExit, onClose, saveMessage }: {
  ...
  async function copy(promotion = false) {
    const candidate = { ...settings, worksLayout: experienceCanonical.worksLayout }
  ...
  return <aside className="development-comparison" id="development-comparison" aria-label="개발 비교" lang="ko">
    ...
    <table className="development-values"><caption>현재 배포값과 개발 선택</caption><thead><tr><th scope="col">항목</th><th scope="col">현재 배포</th><th scope="col">개발 선택</th></tr></thead><tbody>{experienceKeys.filter(key => key !== 'worksLayout').map(key => <tr key={key} data-changed={settings[key] !== experienceCanonical[key]}>
