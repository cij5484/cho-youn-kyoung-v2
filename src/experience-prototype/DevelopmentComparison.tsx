...
export function DevelopmentComparison({ scope = 'home', settings, onChange, onJump, onReset, onLoad, onExit, onClose, saveMessage }: {
  ...
  return <aside className="development-comparison" id="development-comparison" aria-label="개발 비교" lang="ko">
    ...
    {scope === 'works' ? <fieldset><legend>WORKS 구성</legend><div className="development-choices">{(['current', 'spatial-helix'] as const).map(value => <label key={value}><input type="radio" name="works-layout" checked={settings.worksLayout === value} onChange={() => onChange({ worksLayout: value })}/><span>{experienceLabel('worksLayout', value)}</span></label>)}</div><p>공간 나선은 검토 후보이며 일반 WORKS는 현재 배포 구성을 유지합니다.</p></fieldset> : <>
