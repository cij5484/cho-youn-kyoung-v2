...
export function DevelopmentComparison({ scope = 'home', settings, onChange, onJump, onReset, onLoad, onExit, onClose, saveMessage }: {
  ...
  return <aside className="development-comparison" id="development-comparison" aria-label="개발 비교" lang="ko">
    ...
    {scope === 'works' ? <fieldset><legend>WORKS 공간 연출</legend><div className="development-choices">{(['z-depth', 'wave-path', 'stack-flow'] as const).map(value => <label key={value}><input type="radio" name="works-layout" checked={settings.worksLayout === value} onChange={() => onChange({ worksLayout: value })}/><span>{experienceLabel('worksLayout', value)}</span></label>)}</div><p>같은 6개 작품·같은 탐색 목록으로 비교합니다. 초기화하면 현재 배포 구성으로 돌아갑니다.</p></fieldset> : <>
