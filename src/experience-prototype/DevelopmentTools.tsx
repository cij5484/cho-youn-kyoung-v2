...
export function DevelopmentTools({ host, settings }: { host: HTMLElement | null; settings: ComparisonSettings; locale: 'ko' | 'en' }) {
  ...
  function selectedQuery(options: ComparisonSettings | typeof experienceCanonical) {
    return comparisonSearch(location.search, { ...options, dev: true })
  }
  function mainFocus() { document.getElementById('interaction-main')?.focus({ preventScroll: true }) }
  ...
  function reset() {
    const options = { ...experienceCanonical, worksLayout: settings.worksLayout }
    const saved = saveExperienceDraft(options)
    setSaveMessage(saved ? '현재 배포값으로 개발 선택과 저장값을 초기화했습니다.' : '현재 배포값으로 돌아갑니다. 브라우저 저장은 제한되어 있습니다.')
    update(comparisonSearch(resetComparison(location.search), { ...experienceCanonical, dev: true }))
  }
  function load() {
    ...
    update(comparisonSearch(resetComparison(location.search), { ...(draft.options ?? experienceCanonical), dev: true }))
  }
  function exit() {
    update(resetComparison(location.search, true)); setOpen(false); mainFocus()
  }
  ...
  return <>
    ...
    {settings.available && open && <PanelBoundary key="panel" onReset={exit}><Suspense fallback={<p className="development-launcher" role="status">개발 비교 준비 중…</p>}><div onKeyDown={event => { if (event.key === 'Escape' && !document.querySelector('dialog[open]')) { event.stopPropagation(); close() } }}>
      <Panel settings={settings} onChange={change} onJump={jump} onReset={reset} onLoad={load} onExit={exit} onClose={close} saveMessage={saveMessage}/>
