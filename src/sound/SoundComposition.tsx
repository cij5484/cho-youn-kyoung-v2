...
export function SoundComposition({ locale = 'ko', source = homeSoundSource, visual, trail, activity, violet, preset }: { locale?: 'ko' | 'en'; source?: SoundSource; visual: SoundVisual; trail: ContactTrail; activity: ContactActivity; violet: ContactViolet; preset: BowPresetName }) {
  ...
  return <div ref={root} className="sound-experience" data-sound-ready="false" data-sound-static="false">
    ...
    <section className="sound-region" aria-labelledby="sound-title">
      ...
      <SoundSurface key={JSON.stringify([source.src,source.status,source.duration])} root={root} locale={locale} source={source} visual={visual} trail={trail} activity={activity} violet={violet} preset={preset} />
    </section>
  </div>
}
