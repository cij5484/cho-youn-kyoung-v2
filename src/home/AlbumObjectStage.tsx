...
export function PaperAlbumObject({ album }: AlbumObjectAdapterProps) {
  return <div className="paper-album" style={{ '--album-aspect': album.aspect } as CSSProperties} data-renderer="paper-2.5d" data-object-id={album.reference.id} aria-hidden="true">
    <div className="album-face album-front" data-asset-identity={`${album.reference.id}:front`}><img src={album.front} alt="" draggable="false" loading="lazy"/></div>
    <div className="album-face album-back" data-asset-identity={`${album.reference.id}:back`}><img src={album.back} alt="" draggable="false" loading="lazy"/></div>
...
export function AlbumObjectStage({ selected, onSelect, locale, renderObject = props => <PaperAlbumObject {...props}/> }: {
  ...
  const album = homeAlbums[selected], surface = useRef<HTMLDivElement>(null), turn = useAlbumMotion(surface, album.reference.id)
  return <section id="album-object" className="album-object-scene" data-home-scene="05" data-selected-album={album.reference.id} aria-labelledby="album-object-heading">
    ...
    <div className="album-presentation" data-selected-album={album.reference.id} style={{'--exchange-duration':`${albumObjectTuning.exchangeDuration}ms`} as CSSProperties}>
      <div className="album-light-memory" aria-hidden="true"/>
