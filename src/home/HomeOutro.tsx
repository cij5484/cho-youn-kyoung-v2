...
export function HomeOutro({ locale }: { locale: 'ko' | 'en' }) {
  ...
  return <footer ref={root} className="home-outro" data-home-scene="08" aria-labelledby="outro-heading">
    ...
    <SceneRevisitIndex/>
    <div className="outro-bottom"><p>© {new Date().getFullYear()} CHO YOUN KYOUNG</p><div className="outro-credit"><button type="button" onClick={() => setCredit(value => Math.min(2, value + 1))} aria-label={credit ? '제작자의 작은 인사' : '제작자 서명 보기'} aria-expanded={credit > 0}>{credit ? 'Sou.P' : 'Made with care ↗'}</button><span role="status">{credit === 2 ? '사랑하는 소울이와 하울이 아빠' : ''}</span></div></div>
  </footer>
}
