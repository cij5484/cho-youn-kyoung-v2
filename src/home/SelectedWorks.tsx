  ...
  return <section ref={root} id="selected-works" className="works-scene" data-home-scene="04" data-pin="true" aria-labelledby="works-heading">
    <div className="works-sticky">
      ...
      <div className="works-board">
        {selectedWorks.map((work,i)=>{const img=homeImage(work.image);return <article className={`selected-work work-${i+1}`} key={work.reference.id} data-work-index={i} data-content-id={work.reference.id} data-asset-id={work.image.asset.id} style={{'--work-aspect':img.width!/img.height!} as CSSProperties}>
