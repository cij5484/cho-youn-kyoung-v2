...
export function useWorksRibbon(ref: RefObject<HTMLElement | null>, onActive: (index: number) => void) {
  ...
  useEffect(() => {
    ...
    function paint(now: number) {
      ...
      // The tall mobile grid needs a viewport-sized free path, not the offscreen center of the entire board.
      ...
      const alpha = smooth((handoff - .45) / .15) * (1-smooth((endMotion-.91)/.09))
