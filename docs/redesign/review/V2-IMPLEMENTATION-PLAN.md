...
영문은 인간 검토를 거친 공식 콘텐츠로만 완료 처리한다. 미완료 EN은 preview에서 상태를 분명히 하고 출시 승인에서 막는 안을 제안한다. 한국어 문장을 영문으로 가장하거나 자동 번역으로 채우지 않는다. 부분 EN 공개를 원하면 scope를 별도로 승인받는다. locale 변경은 동일 work id를 유지하며 locale만 바꾼다.
